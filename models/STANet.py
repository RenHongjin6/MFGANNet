...
class mynet3(nn.Module):
    def __init__(self, backbone='resnet34', output_stride=32, f_c=64, freeze_bn=False, in_c=3):
...
class models(nn.Module):
    def __init__(self):
        ...
        self.mynet3 = define_F(in_c=3,f_c=64,type='mynet3')
        self.CSDA = CDSA(in_c=128,ds=4,mode='PAM')
        self.conv = nn.Conv2d(128,64,kernel_size=3,stride=1,padding=1,bias=False)
        self.conv1=nn.Conv2d(64,2,kernel_size=3,stride=1,padding=1,bias=False)
    ...
    def forward(self,x1,x2):
        ...
        # dist = feat_A + feat_B
        dist = F.interpolate(out, size=(H,W), mode='bilinear', align_corners=True)
        dist = self.conv(dist)
        dist = self.conv1(dist)
        return dist
        # return out
