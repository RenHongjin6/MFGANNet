...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.conv = nn.Conv2d(768,3,kernel_size=1,stride=1,bias=False)
    ...
    def forward(self, x):
        ...
        x, H, W = self.layer1(x, H, W)
        print(x.shape)
        x1_1 = x.view(-1,96,H,W)

        x = self.a(x, H, W)
        H, W = (H + 1) // 2, (W + 1) // 2
        x, H, W = self.layer2(x, H, W)
        x2_1 = x.view(-1,192,H,W)

        x = self.b(x, H, W)
        H, W = (H + 1) // 2, (W + 1) // 2
        x, H, W = self.layer3(x, H, W)
        x3_1 = x.view(-1,384,H,W)

        x = self.c(x,H,W)
        H, W = (H + 1) // 2, (W + 1) // 2
        x, H, W = self.layer4(x, H, W)
        x4_1 = x.view(-1,768,H,W)
        #     if i == 0:
        #         x, H, W = layer(x, H, W)
        #         x1_1 = x.view(-1, 96, H, W)
        #     elif i == 1 :
        #         a = PatchMerging(dim=96)
        #         x = a(x,H,W)
        #         # print(x.is_cuda)
        #         H, W = (H + 1) // 2, (W + 1) // 2
        #         x, H, W = layer(x, H, W)
        #         x2_1 = x.view(-1, 192, H, W)
        #     elif i == 2 :
        #         b = PatchMerging(dim=192)
        #         x = b(x,H,W)
        #         H, W = (H + 1) // 2, (W + 1) // 2
        #         x, H, W = layer(x, H, W)
        #         x3_1 = x.view(-1, 384, H, W)
        #     elif i == 3 :
        #         c = PatchMerging(dim=384)
        #         x = c(x,H,W)
        #         H, W = (H + 1) // 2, (W + 1) // 2
        #         x, H, W = layer(x, H, W)
        #         x4_1 = x.view(-1, 768, H, W)
        ...
        # return F.interpolate(x, [224,224], mode='bilinear', align_corners=False)
